# Reins Project Configuration

Copy this file to `docs/reins/config.md` in your project to customize Reins behavior.
...
If not set, Reins auto-detects from project signals (.github/, .jira/, .linear/, docs/ structure).
...
If not set, Reins auto-detects from project structure.

## Constraint Overrides

Add rules to override or extend Reins defaults:
...
If `false`, Reins Grill only runs when explicitly invoked ("grill this"), never auto-suggested before a casual architecture decision locks in.
...
Disable specific Reins features:
