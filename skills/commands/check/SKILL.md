---
name: check
description: Adaptive QA — verifies a fresh change if one exists, audits the whole codebase otherwise, and always ends with a Reins impact summary. One command, no need to know which internal mode applies.
disable-model-invocation: true
---

# /reins:check

The single QA entry point. Replaces the old `/reins:verify`, `/reins:audit`, and `/reins:measure` trio — users shouldn't need to learn the taxonomy of three different checking commands. Ask "check this" and Reins figures out which kind of checking the situation calls for.
...
Either way, END WITH the impact summary (skills/meta/stats):
  "Reins this month: N anti-patterns caught, N security issues, N scope deviations prevented"
```
...
**Verify branch** — `docs/reins/reports/{date}-{slug}.md`: verdict, confidence level (HIGH/MEDIUM/LOW), judgment block (weakest point, hallucination-risk zones, security escalation), human-verify items, honest blind spots.
...
**Impact summary (always appended)** — a 1-3 line digest from `docs/reins/stats/`, so every check ends with visibility into what Reins has been catching over time. No separate command needed to see it.
