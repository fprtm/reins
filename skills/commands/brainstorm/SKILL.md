...
The pre-everything command. For "gua ada ide tapi masih kabur" — the idea isn't sharp enough to interrogate decisions about it (`/reins:discover`), let alone design or build it. This is where it ripens.
...
**Isn't**: an intake funnel. No frontier/rounds mechanic, no plan file, no spec, no DoD, no doc generation, no pipeline. The Reins execution machinery stays completely OFF. Nothing about this session pressures the user toward building anything.
...
4. **Let it end wherever it ends.** A brainstorm that concludes "this idea isn't worth it" or just trails off is a *successful* session. Never steer toward "so, ready to build?"

## Output — Optional, Only on Request

If (and only if) the user wants to keep the result, write a short idea brief to `docs/reins/design/{NNN}-{slug}-idea.md`:
...
When the idea firms up and the user signals they want to get serious ("oke gua mau seriusin ini"), offer — don't auto-start — the next step: `/reins:discover` to interrogate the real decisions, which flows into design → implement → check. The idea brief (if written) feeds discover so nothing discussed gets re-asked.

## Position in the Command Flow

```
/reins:brainstorm   idea is fog → shape it          (this command)
/reins:discover     decisions forming → interrogate them
/reins:design       solution shaping → architecture + spec (+ auto ticket split if large)
/reins:implement    build under guardrails
/reins:check        verify the change / audit the codebase + impact summary
```
