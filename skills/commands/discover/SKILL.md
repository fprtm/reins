---
name: discover
description: Investigate a decision — architecture, scope, or direction — before it locks in. Runs a frontier/round interview backed by Reins's own judgment engines.
disable-model-invocation: true
---

# /reins:discover

Manual entry point to `skills/think/grill/SKILL.md`. Use when you want to interrogate a decision before it becomes a plan — not after.

## What Happens When Called

Runs the frontier/round interview mechanic: map the decision as a design tree, ask the whole "askable now" batch each round (numbered, each with a recommendation), wait for answers, recompute, repeat until nothing is left assumed.

Recommendations come from Reins's own judgment engines, not generic reasoning:
- Architecture questions → `skills/think/arch-analyzer/`
- Scope questions → `skills/think/scope-guard/`
- Complexity questions → `skills/think/complexity-analyzer/`
- Security questions → `skills/constraints/[domain]/`

Facts the environment already knows (existing patterns, git history) are looked up by the agent, never asked of the user. Only real decisions go to the user.

## Output

- New domain terms → `docs/reins/glossary.md`
- Decisions that pass the rule-of-three gate (hard to reverse + surprising + real trade-off) → `docs/reins/decisions/`
- **No plan file is written.** If the session ends with "let's build it," the shared understanding feeds into the plan the next time `/reins:implement` or the orchestrator runs — it is not re-asked.

## Full Behavior

See `skills/think/grill/SKILL.md` for the complete mechanic, judgment heuristics, and mode interactions.
