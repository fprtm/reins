...
| Phase | Setting |
|-------|---------|
| Elicitation | 0-1 questions. Auto-infer from context. Only ask if genuinely blocked. |
...
| Plan file | Written to `docs/reins/plans/current.md` silently. Auto-approved. No wait. |
| Change plan | Auto-declare. No approval needed. Note deviations in summary. |
| Doc generator | Generate docs silently. Available in docs/reins/ for later review. |
...
2. They get a result that's better than it would have been without Reins.
...
- Lecture (long reports about what Reins caught)
...
Plan is written to `docs/reins/plans/current.md` automatically but NOT shown to user. Auto-approved. After task, archived to `docs/reins/plans/archive/`. User can review later if curious.

## Document Handling

Docs (FSD, SDD, etc.) are generated silently to `docs/reins/design/`, `docs/reins/erd/`, etc. Not mentioned to user unless they ask. Available for later review.
