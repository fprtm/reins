...
Feeds into the visual report (see `SKILL.md`) when there are enough distinct candidates to warrant one, or directly into `/reins:discover` (`skills/think/grill/`) for a frontier/round session to work through the chosen candidate's remaining open questions.
