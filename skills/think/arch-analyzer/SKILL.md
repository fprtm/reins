...
Before flagging an inconsistency as a problem, check `docs/reins/decisions/` — the "inconsistency" might be a deliberate, already-logged trade-off (e.g., an ADR explaining why one module intentionally breaks the layering rule). If a logged decision covers it, don't re-flag it as an issue.
...
For everything else, the standard single-pass analysis above is enough. This is not the default architecture flow — it's reserved for decisions worth the extra dispatch cost.
...
Feeds into the visual report below when there are enough distinct candidates to warrant one, or directly into `/reins:discover` (`skills/think/grill/`) for a frontier/round session to work through the chosen candidate's remaining open questions.
...
- **Controlled vocabulary only** — use glossary terms (`docs/reins/glossary.md`) and Reins's own vocabulary (module, interface, seam, adapter). Never substitute loose synonyms like "component," "service," "boundary" inconsistently across cards.
...
The user picks a candidate from the report → hand off to **Reins Grill** (`skills/think/grill/`) to work through the specifics (constraints, seam shape, migration order) via the frontier/round interview. The report is the menu; grilling is where the decision actually gets made.
...
- If user asks for simpler: comply immediately. Reins advises, user decides.
