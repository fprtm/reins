...
Most bad AI-agent outcomes trace back to a decision made casually in conversation ("let's use microservices", "we'll need real-time sync") that never got interrogated before it became a plan. By the time the orchestrator writes `docs/reins/plans/current.md`, the direction is often already locked. Grill runs *before* that lock-in, at the moment the decision is still soft.
...
When detected, Reins asks once: *"This sounds like an architecture-level decision. Want to grill it first before we build?"* — user can decline, and Reins proceeds straight to plan-writing without complaint.
...
## Where Recommendations Come From — This Is What Makes It "Reins" Grill

Generic grilling gives recommendations from general reasoning. Reins Grill routes each question type through Reins's own judgment engines:
...
1. **Glossary update**: any new domain term that surfaced gets written to `docs/reins/glossary.md` (see `skills/meta/glossary/`) — live, not batched.
...
3. **Hand-off to plan**: if the user follows up with "build this" / "let's do it", the shared understanding from the grill session feeds directly into `docs/reins/plans/current.md`. THINK-phase questions already settled in the grill are NOT re-asked by elicitation.
...
2. Never ask the user something Reins (or a sub-agent) can look up.
...
5. Grilling never writes `docs/reins/plans/current.md` directly. It writes glossary/ADR entries only. Plan-writing stays the orchestrator's job, triggered by an explicit build signal.
