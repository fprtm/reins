...
Ambiguous terminology is a root cause of specification ambiguity (problem #1 in Reins's original analysis). If "user" sometimes means "authenticated account" and sometimes means "any visitor," every downstream decision built on that word inherits the ambiguity. The glossary makes the project's vocabulary explicit and enforces one canonical word per concept.

## File

`docs/reins/glossary.md` — created lazily, on the first term that needs pinning down. Don't pre-populate it with a generic dictionary.
...
- **Reins Grill** (`skills/think/grill/`): new terms surfacing during a grill session get written here live.
