...
**Constraint overrides always pass the gate automatically** — a user overriding an Reins rule is by definition surprising to a future reader and worth recording.

## Log Entry Format

Each decision is a separate file in `docs/reins/decisions/`.
...
4. After writing, update `docs/reins/index.md` with a link to the new decision.
5. If this is the first decision, create the `docs/reins/decisions/` directory.
...
When starting a new task, check `docs/reins/decisions/` and `docs/reins/glossary.md` for relevant prior context:
