...
Track what Reins does per task. Powers vibe mode footer and historical stats dashboard.
...
After every task, Reins records a stats entry.
...
```
---
Reins: 2 anti-patterns fixed | 1 security issue caught | 4 files changed | confidence: HIGH
```

Rules:
- Only show if Reins actually did something (no empty footer)
...
Append to `docs/reins/stats/{YYYY-MM}.md`:
...
After appending stats, update `docs/reins/index.md` with:
...
1. **Never fabricate stats** — Only count what Reins actually caught, not what might exist.
2. **Append-only per task** — Don't edit previous entries. Monthly summary recalculated.
3. **Stats are evidence** — They prove Reins's value. Accuracy matters.
