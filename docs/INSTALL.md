...
That's it. Reins registers as a plugin — the orchestrator and all 7 slash commands (`/reins:discover`, `/reins:decompose`, `/reins:design`, `/reins:implement`, `/reins:verify`, `/reins:audit`, `/reins:measure`) become available immediately.

**Note**: this method only sets up the skill/command layer. To also get the pre-commit hook, CI workflow, and `docs/reins/` project files (glossary, decisions, plans, etc.), run the manual installer once with `--with-hooks --with-ci --with-templates` (see below) — it's safe to run alongside a plugin install.
...
```bash
git clone https://github.com/fprtm/reins
cd reins
```
...
|-------|---------|-------------|
| Claude Code (your user account, all projects) | `./install/install.sh --agent claude` | `~/.claude/commands/reins/` |
| Claude Code (this project only) | `./install/install.sh --agent claude-proj` | `.claude/commands/reins/` |
| Codex CLI | `./install/install.sh --agent codex` | `.agents/skills/reins/` + `AGENTS.md` |
| OpenCode | `./install/install.sh --agent opencode` | `.opencode/skills/reins/` + `AGENTS.md` |
...
Run this **from inside the project you want Reins to guard**, not from the `reins/` clone itself — unless you're installing user-wide (`--agent claude`), in which case it doesn't matter.

> **Cursor note**: Cursor has limited skill support, so only the orchestrator is copied as a single rules file. For full Reins (all 48 skills, 7 commands), use Claude Code or Codex.
...
Creates `docs/reins/` in your project: `config.md`, `glossary.md`, `memory.md`, `index.md`, and empty `decisions/`, `plans/`, `tickets/`, `reports/`, `design/`, `test-plans/`, `dod/`, `stats/`, `erd/` directories. Without this, Reins still works, but has nowhere to persist plans, decisions, or stats — it'll create these on first use anyway, so this step is just "set it up now vs. let it happen automatically."
...
- `--with-ci` — copies a GitHub Actions workflow to `.github/workflows/reins-check.yml` that runs the same class of checks on every PR.
...
```bash
cd reins   # the cloned repo, not your project
...
`--update` overwrites the installed skill files but leaves your project's `docs/reins/config.md` (and everything else in `docs/reins/`) untouched — your mode defaults, constraint overrides, and history are preserved.
...
Removes the installed skill files, the pre-commit hook (if it was Reins's), and the CI workflow (if present). **Does not** touch `docs/reins/` in your project — your decisions, plans, and stats are left in place. Delete that directory manually if you want a full clean removal.
...
Check `./install/install.sh --version` — you may be running an old copy. `git pull` in the `reins/` clone first.
...
**Slash commands (`/reins:discover` etc.) don't show up in Claude Code**
...
**Want to confirm what mode/config Reins is using in a project**
Check `docs/reins/config.md` in that project — if absent, Reins is using defaults (standard mode, auto-detected domain/SDLC).
